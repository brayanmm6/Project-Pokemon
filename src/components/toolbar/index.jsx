import { PokemonFilter, StyledBall } from "../../styled-components/filter"
...
const Toolbar = (props) => {
    ...
    return(
        <StyledToolbar theme={props.theme} >
            ...
            <StyledSearchBox>
                <StyledSearchInput type="search" name="pokemon-search" id="pokemon-search" placeholder="Type a Pokémon name." onKeyDown={ (enter) =>  enter.key === "Enter" && setSearch({data: enter.target.value})} onChange={ ( event ) => event.target.value.length === 0 && setSearch({data: "default"})}/>
